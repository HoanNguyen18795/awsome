...
import com.hoannk.config.KafkaProperties;
...
class AwsomeKafkaResourceIT {
    ...
    void setup() {
        ...
        AwsomeKafkaResource kafkaResource = new AwsomeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/awsome-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/awsome-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
